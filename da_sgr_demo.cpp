...
int main(int argc, char * argv[])
{
	...
	for (int g=0; g<nGroups; g++)
	{
		...
		{
			snprintf(filename,512,"%s_%04d.bin",sFile[g],s);
			std::ifstream file(filename);  
			if (!file.is_open()) continue;
			printf("   SET %4d  FILTERS ",s);
			int nFilters = 0;
			while(DSP.LoadingFilters()) 
			{
			...
			{
				{
					...
					if (nMagic!=0xDAC0EF01) break;
					//printf("  FILTER %08X %4d %4d %4d %4d\n",nMagic,nFilt,nLength,nIn,nOut);
					if (nLength && nIn && nOut)
					{
						file.read((char *)Filt,nLength*sizeof(float));
						if (nIn>DSP.Inputs() || nOut>DSP.Outputs()) { printf("ERROR: Filter IN=%d OUT=%d exceeds DSP limits\n",nIn,nOut); continue; };
						DSP.LoadFilter(nIn-1,nOut-1,nLength,Filt,g);
	...
	int nRotScale = std::max(1,(int)(72.0 / (nRots-1) + 0.5));
	int nRotMax   = nRots*nRotScale;
	int nRotCent  = 0;

    int nReverbs  = nSets[1];
	...
	while(g_running && DSP.Running())
	{
		...
		bool  bReverb = false;			// Toggle for when reverb (fourth on row 3) held down
		bool  bReverbChanged = false;	// Set if the reverb has changed
		int   nReverb = 0;				//
		...
		while (g_running && DSP.Running())
		{
			...
			{	
				...
				{
					if (message[2]==0x7F) { bReverb = true;  bReverbChanged = false; };
					if (message[2]==0x00) { bReverb = false; if (!bReverbChanged) { nReverb = 0; }; };
				}
				...
				if (message[0]==0xB0 && message[1]==0x10)
				{
					...
					{
						if (message[2] & 0x40) nReverb -= 1; 
						else                   nReverb += 1;
						nReverb = std::max(0,std::min(nReverbs-1,nReverb));
						bReverbChanged = true;
					}
					else 
					{
				...
				DSP.SetFilterSet((Row1*8 + Row2)*nRots + r,0);
				DSP.SetFilterSet(nReverb,1);		
				printf("ROW1 %d  ROW2 %d    ROT %d    FOCUS %3d   NROT %3d   VOL %5.3f   REVERB %2d  DSP SETS %3d / %3d\n",Row1,Row2,r,nFocus,nRot,fVol,nReverb,DSP.GetFilterSet(0),DSP.GetFilterSet(1));
			}	
