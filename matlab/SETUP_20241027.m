...
hl = conv(hl,impz(b,a,2048));  hl = hl(1:1024);  hl= fade(hl,[0 .1]);
...
clf;
Spectra([hl; zeros(10000,1)]); hold on;
Spectra([hw; zeros(10000,1)]); 
Spectra([ht; zeros(10000,1)]); 
Spectra([hw + [ht; zeros(length(hw)-length(ht),1)]; zeros(10000,1)]); 
grid on;
axis([10 24000 -40 20]);
...
GENELEV = [ zeros(1,7) ];
GENAZIM = [ -30 0 30 90 150 210 270 ];

SG1ELEV = kron([ zeros(1,16) 0 30 0 30 0 30 0 30 ],[1 1]);
SG1AZIM = kron([ -45 -40 -35 -25 -20 -15 -10 -5 5 10 15 20 25 35 40 45 60 60 120 120 240 240 300 300 ],[1 1]);
